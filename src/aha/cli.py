"""
AHA CLI — Asynchronous Human & AI for AI coding agents.
"""
...
# Config lives in ~/.aha/
AHA_HOME = Path.home() / ".aha"
CONFIG_PATH = AHA_HOME / "config.json"
...
def _save_config(config: dict) -> None:
    AHA_HOME.mkdir(parents=True, exist_ok=True)
...
def _templates_dir() -> Path:
    ...
    # hatch shared-data puts files in {prefix}/share/aha/templates
    for base in [Path(sys.prefix), Path(sys.exec_prefix)]:
        installed_path = base / "share" / "aha" / "templates"
        if installed_path.exists():
            return installed_path
    # uv tool install: {tool_dir}/share/aha/templates
    cli_path = Path(__file__).resolve()
    # Walk up from site-packages to find share/
    for parent in cli_path.parents:
        candidate = parent / "share" / "aha" / "templates"
        if candidate.exists():
            return candidate
    raise FileNotFoundError("AHA templates not found. Try reinstalling: uv tool install --force ...")


def _is_mpm_hook_entry(entry: dict) -> bool:
    """Check if a hook entry contains AHA hooks (references .aha/)."""
    return any(".aha/" in h.get("command", "") for h in entry.get("hooks", []))


def _merge_settings(dest_path: Path, template_path: Path) -> None:
    """Merge AHA hooks from template into existing settings.json, preserving user hooks."""
...
@click.pass_context
def main(ctx):
    """AHA — Asynchronous Human & AI for AI coding agents."""
    if not CONFIG_PATH.exists() and ctx.invoked_subcommand not in ("onboard",):
        click.echo("AHA has not been initialized yet.")
        if click.confirm("Run initial setup now?"):
            ctx.invoke(onboard)
        else:
            click.echo("Run 'aha onboard' when ready.")
            raise SystemExit(0)
...
def onboard(ctx):
    ...
    click.echo("Welcome to AHA! Let's set up your configuration.\n")
    ...
    config.update({
        ...
        "tmux_prefix": config.get("tmux_prefix", "aha"),
    ...
    click.echo("Run 'aha dashboard' to start.")
...
def dashboard(foreground, no_open):
    """Start the AHA dashboard server."""
    config = _load_config()
    if not config:
        click.echo("Run 'aha onboard' first to set up configuration.")
        return
    ...
    if foreground:
        click.echo(f"AHA Dashboard → {url}")
        click.echo("Press Ctrl+C to stop.\n")
        if not no_open:
            _open_browser(url)
        try:
            subprocess.run([sys.executable, "-m", "aha.dashboard.server"], check=True)
        except KeyboardInterrupt:
            click.echo("\nDashboard stopped.")
        return

    # Background (default)
    pid_file = AHA_HOME / "dashboard.pid"
    if pid_file.exists():
    ...
    log_file = AHA_HOME / "dashboard.log"
    proc = subprocess.Popen(
        [sys.executable, "-m", "aha.dashboard.server"],
        start_new_session=True,
        stdout=open(log_file, "a"),
        stderr=subprocess.STDOUT,
    )
    pid_file.write_text(str(proc.pid))
    click.echo(f"AHA Dashboard → {url} (PID {proc.pid})")
...
def init(path):
    """Initialize AHA in a project directory."""
    if path is None:
    ...
    # Copy .aha/ structure
    mpm_dir = project_dir / ".aha"
    if mpm_dir.exists():
        click.echo(f"AHA already initialized in {project_dir}")
        click.echo("Re-syncing scripts and rules...")
    else:
        click.echo(f"Initializing AHA in {project_dir}")
    ...
    # Merge AHA hooks into settings.json (preserve user hooks)
    _merge_settings(
    ...
    for d in [".aha/data/current", ".aha/data/past", ".aha/data/review", ".aha/docs"]:
        (project_dir / d).mkdir(parents=True, exist_ok=True)

    # Make scripts executable
    scripts_dir = project_dir / ".aha" / "scripts"
    ...
    click.echo(f"✓ AHA initialized in {project_dir}")
    click.echo(f"✓ Registered in {CONFIG_PATH}")
    click.echo()
    click.echo("Next: set up your project with the planner agent. Copy and run:")
    click.echo()
    click.echo(click.style(f"  cd {project_dir} && claude --agent aha-planner /aha-init", fg="cyan", bold=True))
    click.echo()
    ...
    try:
        ...
        if not port:
            raise KeyError
        _req.delete(f"http://localhost:{port}/api/sessions/_aha-init", timeout=2)
...
def disable(path):
    """Remove AHA from a project (preserves task data)."""
    project_dir = Path(path).resolve()
    ...
    if scripts_dir.exists():
        ...
        removed.append(".aha/scripts/")

    # Remove Claude Code integration
    for p in [
        project_dir / ".claude" / "rules" / "aha-workflow.md",
    ]:
        if p.exists():
            p.unlink()
    ...
    for d in [
        # Current skills
        project_dir / ".claude" / "skills" / "aha-next",
        project_dir / ".claude" / "skills" / "aha-autonext",
        project_dir / ".claude" / "skills" / "aha-init",
        project_dir / ".claude" / "skills" / "aha-init-uiux",
        project_dir / ".claude" / "skills" / "aha-task-write",
        project_dir / ".claude" / "skills" / "aha-office-hour",
        project_dir / ".claude" / "skills" / "aha-plan-ceo-review",
        project_dir / ".claude" / "skills" / "aha-plan-eng-review",
        project_dir / ".claude" / "skills" / "aha-plan-design-review",
        project_dir / ".claude" / "skills" / "aha-review-functional",
        project_dir / ".claude" / "skills" / "aha-review-code",
        project_dir / ".claude" / "skills" / "aha-review-uiux",
        project_dir / ".claude" / "skills" / "aha-recycle",
        project_dir / ".claude" / "skills" / "aha-ui-ux-pro-max",
    ...
    # Remove AHA hooks from settings.json (keep user hooks)
        ...
        try:
            ...
            for event in list(hooks.keys()):
                hooks[event] = [
                    ...
                    if not any(".aha/" in h.get("command", "") for h in entry.get("hooks", []))
                ]
                if not hooks[event]:
                    del hooks[event]
            if hooks:
            ...
            removed.append(".claude/settings.json (AHA hooks removed)")
        except Exception:
            pass
    ...
    click.echo("Task data preserved in .aha/data/ and .aha/docs/")
...
def status():
    ...
    if not config:
        click.echo("AHA not configured. Run 'aha onboard' first.")
        return
...
def port(port_number):
    ...
    pid_file = AHA_HOME / "dashboard.pid"
    if pid_file.exists():
        click.echo("Restart dashboard to apply: aha stop && aha dashboard")
...
@click.pass_context
def uninstall(ctx):
    """Remove AHA from all projects and clean up global config."""
    config = _load_config()
    projects = config.get("projects", [])

    if not projects and not AHA_HOME.exists():
        click.echo("Nothing to uninstall.")
        return

    if not click.confirm(f"This will disable AHA in {len(projects)} project(s) and remove {AHA_HOME}. Continue?"):
        return
    ...
    if AHA_HOME.exists():
        shutil.rmtree(AHA_HOME)
        click.echo(f"\nRemoved {AHA_HOME}")

    click.echo("\nMPM uninstalled. Run this to complete:")
    click.echo("  uv tool uninstall aha")
