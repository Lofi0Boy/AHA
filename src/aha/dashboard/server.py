"""
AHA Dashboard — web server.

Run from the AHA root:
...
from aha.gateway.file_watcher import start_file_watcher
from aha.gateway.session_manager import (
    capture_pane,
    cleanup_all,
    create_session,
    get_all_sessions,
...
from aha.dashboard.projects import (
...
IDEAS_PATH = AHA_HOME / "ideas.json"
...
def api_docs_list(project_name):
    """List all .md files in a project directory (including .aha/docs/)."""
    ...
    for md_path in sorted(d.rglob("*.md")):
        ...
        parts = rel.parts
        if any(p.startswith(".") and p != ".aha" for p in parts):
            continue
...
def api_promote_idea(idea_id):
    ...
    # Create .aha structure
    mpm_data = project_dir / ".aha" / "data"
    mpm_docs = project_dir / ".aha" / "docs"
...
def api_v2_complete_review(project_name, task_id):
    ...
    cmd = ["python3", str(d / ".aha" / "scripts" / "task.py"), "complete", task_id, verdict]
    if comment:
        cmd += ["--comment", comment]
...
def api_v2_review_screenshots(project_name, task_id):
    ...
    reviews_dir = d / ".aha" / "data" / "reviews"
...
def api_v2_review_file(project_name, filename):
    """Serve a screenshot file from .aha/data/reviews/."""
...
def api_autonext_state():
    ...
    for project_path in config.get("projects", []):
        ...
        if not d.is_dir():
            continue
        sf = d / ".aha" / "data" / "autonext-state.json"
        if sf.exists():
            try:
...
CLI_CONFIG_PATH = Path.home() / ".aha" / "config.json"
...
def api_init_terminal():
    ...
    # Auto-type aha init
    _run(["tmux", "send-keys", "-t", tmux_name, "aha init", "Enter"])
...
def api_register_project():
    ...
    if not (Path(path) / ".aha").is_dir():
        return jsonify({"error": "Not an AHA project (missing .aha/)"}), 400
...
def start_server():
    ...
    SHUTDOWN_MARKER = AHA_HOME / ".server_shutdown_at"
    GRACE_PERIOD = 60
    ...
    if SHUTDOWN_MARKER.exists():
        try:
            ...
            if elapsed > GRACE_PERIOD:
                print(f"AHA: server was down {elapsed:.0f}s (>{GRACE_PERIOD}s) — cleaning orphan sessions")
                should_cleanup = True
            else:
                print(f"AHA: server was down {elapsed:.0f}s (<{GRACE_PERIOD}s) — keeping sessions alive")
    ...
    print(f"AHA Dashboard → http://localhost:{port}")
