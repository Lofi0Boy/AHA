"""
tmux + ttyd session manager for AHA Gateway.
...
def _load_config() -> dict:
    if not CONFIG_PATH.exists():
        return {"patterns": [], "projects": [], "tmux_prefix": "aha"}
...
def cleanup_all() -> None:
    """Kill all AHA-managed ttyd processes and tmux sessions."""
    config = _load_config()
    prefix = config.get("tmux_prefix", "aha")
...
def reconnect_ttyd() -> None:
    ...
    for name in list_tmux_sessions():
        ...
        try:
            ...
            print(f"AHA reconnect: ttyd for {ttyd_key} on port {port}")
        except RuntimeError as e:
            print(f"AHA reconnect: failed for {ttyd_key} — {e}")
