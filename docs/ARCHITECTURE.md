...
대시보드의 모든 액션은 키보드만으로 접근 가능해야 한다.
키보드 네비게이션은 **레이어 모델**(Board → Terminal → Modal → Dialog → Text Input)과 **존 네비게이션**(컬럼 간 좌우, 아이템 간 상하)으로 구성된다.

상세 설계는 [`docs/keyboard_ux.md`](keyboard_ux.md) 참조.

새 UI 기능을 추가할 때는 반드시 keyboard_ux.md의 체크리스트를 따를 것:
1. 어떤 레이어에 속하는가?
2. 키보드로 어떻게 진입하는가?
3. 포커스 가능 아이템이 추가되는가?
4. Escape로 이전 레이어 복귀가 되는가?
