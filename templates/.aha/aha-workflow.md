...
|-------|------|-----------|
| **Planner** | Foundation docs, phase/goal management, task creation | `claude --agent aha-planner` |
| **Developer** | Task execution: pop, implement, fill result | `claude --agent aha-developer` |
...
- **All task operations go through `task.py`** — never read/write `.aha/data/` JSON directly.
