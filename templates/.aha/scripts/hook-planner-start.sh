...
SCRIPTS_DIR="$CWD/.aha/scripts"
DOCS_DIR="$CWD/.aha/docs"
...
f = Path('$CWD/.aha/data/future.json')
...
import json
from pathlib import Path
past_dir = Path('$CWD/.aha/data/past')
count = 0
if past_dir.exists():
...
if [[ -n "$GAPS" ]]; then
  ...
  echo "→ Run /aha-init to fill missing items. Guide the user through each gap."
  echo ""
fi

if [[ "$REJECTED_COUNT" -gt 0 ]]; then
  echo "**$REJECTED_COUNT rejected task(s) in past.**"
  echo "→ Run /aha-recycle to rewrite and return them to future."
  echo ""
fi
