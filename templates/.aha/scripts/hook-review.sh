...
MARKER_FILE=".aha/data/current/${SID}.reviewed"
SEND_COUNT=0
...
if [[ "$TOTAL_ATTEMPTS" -ge "$MAX_REVIEWS" ]]; then
  python3 .aha/scripts/task.py escalate "$SID" 2>/dev/null || true
  rm -f "$MARKER_FILE"
...
Send to @aha-reviewer via **SendMessage**. Ask the reviewer to determine which reviews to run:
- /review-functional
- /review-code
- /review-uiux

Include this task context in your message:
---
${TASK_CONTENT}
---
...
jq -n --arg p "$PROMPT" \
  '{decision:"block", reason:$p, systemMessage:"🔍 SendMessage to @aha-reviewer"}'
