...
HOOK_INPUT=$(cat)
STATE_FILE=".aha/data/autonext-state.json"
...
SID="$HOOK_SESSION"
CURRENT_FILE=".aha/data/current/${SID}.json"
HAS_CURRENT="false"
CURRENT_STATUS=""
...
if [[ "$HAS_CURRENT" == "true" ]] && [[ "$CURRENT_STATUS" == "dev" ]]; then
  ...
  if [[ -z "$CURRENT_RESULT" ]]; then
    ...
    if [[ $NEW_TASK_ITER -ge $MAX_ITER ]]; then
      ...
      python3 .aha/scripts/task.py update "$SID" result "(auto-failed: max $MAX_ITER iterations reached)" 2>/dev/null || true
      python3 .aha/scripts/task.py update "$SID" memo "Max $MAX_ITER iterations reached without completing verification." 2>/dev/null || true
...
import json
tasks = json.load(open('.aha/data/future.json'))
print(len(tasks))
" 2>/dev/null || echo "0")
  ...
  if [[ "$FUTURE_LEN" -eq 0 ]]; then
    ...
    if [[ "$MODE" != "--all" ]] && [[ "$MODE" != "" ]]; then
      echo "✅ AHA Auto-Next complete ($TASKS_DONE tasks processed). No more tasks."
      rm "$STATE_FILE"
      rm -f ".aha/data/current/${SID}.reviewer-id"
      rm -f ".aha/data/current/${SID}.reviewed"
      exit 0
    fi
  ...
  # Feed next task
  PROMPT="Pop the next task: python3 .aha/scripts/task.py pop $SID
