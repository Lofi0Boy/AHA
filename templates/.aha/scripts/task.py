#!/usr/bin/env python3
"""
AHA Task System CLI — manages task transitions.
...
Usage:
    ...
    task.py recycle <task_id> <new_prompt> [--goal "..."] [--verification "..."]  # rejected past → future
"""
...
CONFIG_PATH = Path.home() / ".aha" / "config.json"
...
try:
    ...
    import msvcrt
    def _lock():
        """Acquire file lock for concurrent access safety (Windows)."""
        LOCK_PATH.parent.mkdir(parents=True, exist_ok=True)
        lock_fd = open(LOCK_PATH, "w")
...
def cmd_recycle(task_id, new_prompt, new_goal=None, new_verification=None):
    """Move a rejected task from past back to future with a new prompt.
    Injects previous attempt context into prompt. Optionally overrides goal/verification."""
    lock = _lock()
    try:
        ...
        # Inject previous attempt context into prompt
        prev_context_parts = ["## Previous attempt (rejected)"]
        if found_task.get('goal'):
            prev_context_parts.append(f"- Goal: {found_task['goal']}")
        if found_task.get('verification'):
            prev_context_parts.append(f"- Verification: {found_task['verification']}")
        if found_task.get('result'):
            prev_context_parts.append(f"- Result: {found_task['result']}")
        hr = found_task.get('human_review') or {}
        if hr.get('comment'):
            prev_context_parts.append(f"- Rejection reason: {hr['comment']}")
        prev_context = "\n".join(prev_context_parts)
        enriched_prompt = f"{prev_context}\n\n{new_prompt}"
        ...
        # Use new goal/verification if provided, otherwise preserve original
        new_task['goal'] = new_goal if new_goal else found_task.get('goal', '')
        new_task['verification'] = new_verification if new_verification else found_task.get('verification', '')
...
def main():
    ...
    if cmd == "pop" and len(sys.argv) >= 3:
        ...
        r_verif = None
        r_args = sys.argv[4:]
        for ri in range(0, len(r_args) - 1, 2):
            if r_args[ri] == "--goal":
                r_goal = r_args[ri + 1]
            elif r_args[ri] == "--verification":
                r_verif = r_args[ri + 1]
        cmd_recycle(sys.argv[2], sys.argv[3], new_goal=r_goal, new_verification=r_verif)
    elif cmd == "status":
        cmd_status()
