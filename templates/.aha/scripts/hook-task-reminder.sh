...
CURRENT_DIR="$CWD/.aha/data/current"

if [ ! -d "$CURRENT_DIR" ]; then
  echo "[AHA Task] No current task. If user requests code changes, send to @aha-planner via SendMessage to create tasks."
  exit 0
fi
