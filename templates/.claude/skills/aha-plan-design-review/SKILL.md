...
Then read:
- **`.aha/docs/UIUX.md`** — the primary review target. This is the UI plan to rate and improve.
- **`.aha/docs/DESIGN.md`** + **`.aha/docs/tokens/`** — all design decisions calibrate against this
- **`.aha/docs/PROJECT.md`** — product context, target users
- CLAUDE.md — project conventions

Map:
* What is the UI scope of the plan? (pages, components, interactions)
* Does DESIGN.md exist? If not, flag as a gap.
* Does UIUX.md exist? If not, there is nothing to review — recommend `/aha-init-uiux` first.
...
- If no DESIGN.md: "No design system found. Recommend running /aha-init-uiux first. Proceeding with universal design principles."
...
Re-run loop: invoke /aha-plan-design-review again → re-rate → sections at 8+ get a quick pass, sections below 8 get full treatment.
...
FIX TO 10: If DESIGN.md exists, annotate with specific tokens/components. If no DESIGN.md, flag the gap and recommend `/aha-init-uiux`.
...
If all passes 8+: "Plan is design-complete. Implementation can begin."
...
- If eng review has not been run: recommend `/aha-plan-eng-review`
- If fundamental product direction gaps were found (initial score below 4/10, major IA problems): consider `/aha-plan-ceo-review`
- The user can update `.aha/docs/DESIGN.md` based on the design decisions made during this review.
