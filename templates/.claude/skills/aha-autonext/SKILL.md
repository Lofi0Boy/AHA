---
name: aha-autonext
description: Automatically work through selected future tasks — work, fill result, and move to next after review.
...
Automatically process tasks from the future queue. Each task: pop → work → fill result → agent review → next.
...
```bash
SID=$(grep "session=" /tmp/aha-hook.log | tail -1 | sed 's/.*session=//')
...
cat > .aha/data/autonext-state.json << STATEEOF
{
  "session_id": "$SID",
  "max_iterations": 3,
  "task_iteration": 0,
...
1. Pop the next task (pass filter from autonext-state.json if set):
   ...
   GOAL=$(python3 -c "import json; s=json.load(open('.aha/data/autonext-state.json')); print(s.get('goal_filter',''))" 2>/dev/null)
   PHASE=$(python3 -c "import json; s=json.load(open('.aha/data/autonext-state.json')); print(s.get('phase_filter',''))" 2>/dev/null)
   POP_ARGS=""
   [ -n "$GOAL" ] && POP_ARGS="--goal $GOAL"
   [ -n "$PHASE" ] && POP_ARGS="--phase $PHASE"
   python3 .aha/scripts/task.py pop ${CLAUDE_SESSION_ID} $POP_ARGS
   ```
...
4. Fill `result`, `changes`, and `memo` via `task.py update`. This auto-transitions to `agent-review`.
...
```bash
rm .aha/data/autonext-state.json
```
