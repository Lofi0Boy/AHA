---
name: aha-init
description: Initialize a new AHA project — Create foundation documents, set up Phase, Goals and Tasks.
---

# Initialize AHA Project
...
After completion, a product spec is saved to `.aha/gstack/design-{datetime}.md`.

### 3b. `/aha-plan-ceo-review`

Strategic review. Challenges scope, maps the 12-month ideal, validates premises.

After completion, a CEO plan may be saved to `.aha/gstack/ceo-plans/`.

### 3c. `/aha-plan-eng-review`

Engineering review. Locks in architecture, data flow, test strategy, edge cases.

### 3d. Write foundation documents

After the three reviews are complete, split the generated documents into `.aha/docs/` foundation documents by topic. **Do NOT summarize, compress, or rewrite any content** — move sections as-is, only separating them into the correct file.

1. **`.aha/docs/PROJECT.md`** — Product planning and design sections. Move the following sections verbatim from the office-hour product spec and CEO review:
...
2. **`.aha/docs/ARCHITECTURE.md`** — Technical architecture sections. Move the following sections verbatim from the eng review:
...
Write to `.aha/docs/VERIFICATION.md` with the following structure:
...
- **If yes:** Run `/aha-init-uiux`. This creates `.aha/docs/DESIGN.md`, `.aha/docs/tokens/`, and `.aha/docs/UIUX.md`, then runs `/aha-plan-design-review` to validate the UI plan.
...
```bash
python3 .aha/scripts/phase.py add "Phase Name" "Verifiable completion state description"
```

This stores the Phase in `.aha/data/phases.json` as structured data.
...
1. Break down into Tasks following `/aha-task-write` skill guidelines (single function, 1–2 evidence items)
...
```bash
# Create Goals
python3 .aha/scripts/phase.py goal-add <phase_id> "Goal description from user perspective"
```

For each Task, invoke `/aha-task-write` — do NOT call `task.py add` directly. The skill ensures consistent structure, foundation doc references, and proper verification methods.

Show the final created list to the user.

## Next: Start development

> Tell the user: "Setup complete. To start development, run the developer agent:"

```bash
cd <projectdir> && claude --agent aha-developer /aha-autonext
```

Or to skip permission prompts:

```bash
cd <projectdir> && claude --dangerously-skip-permissions --agent aha-developer /aha-autonext
```

You can also use `@aha-developer` within an existing session, but `--agent` is recommended — running planner and developer as separate sessions allows parallel execution: developer works on tasks while planner creates new tasks and reviews results in another terminal.
