...
```
Then read CLAUDE.md, `.mpm/docs/` documents, and any existing architecture docs.

**Design doc check:**
```bash
DESIGN=$(ls -t .mpm/gstack/design-*.md 2>/dev/null | head -1)
[ -n "$DESIGN" ] && echo "Design doc found: $DESIGN" || echo "No design doc found"
```
If a design doc exists (from `/mpm-office-hour`), read it. Use it as the source of truth for the problem statement, constraints, and chosen approach. If it has a `Supersedes:` field, note that this is a revised design.

**Mid-session detection:** During Step 0A (Premise Challenge), if the user can't
articulate the problem, keeps changing the problem statement, answers with "I'm not
sure," or is clearly exploring rather than reviewing — offer `/mpm-office-hour`:

> "It sounds like you're still figuring out what to build — that's totally fine, but
> that's what /mpm-office-hour is designed for. Want to run it right now?
> We'll pick up right where we left off."

Options: A) Yes, run /mpm-office-hour now. B) No, keep going.
...
```bash
mkdir -p .mpm/gstack/ceo-plans
```

Before writing, check for existing CEO plans in the ceo-plans/ directory. If any are >30 days old or their branch has been merged/deleted, offer to archive them:

```bash
mkdir -p .mpm/gstack/ceo-plans/archive
# For each stale plan: mv .mpm/gstack/ceo-plans/{old-plan}.md .mpm/gstack/ceo-plans/archive/
```

Write to `.mpm/gstack/ceo-plans/{date}-{feature-slug}.md` using this format:
...
Generated by /mpm-plan-ceo-review on {date}
...
The CEO calling in the designer. Not a pixel-level audit — that's /mpm-plan-design-review and /mpm-design-review. This is ensuring the plan has design intentionality.
...
If this plan has significant UI scope, recommend: "Consider running /mpm-plan-design-review for a deep design review of this plan before implementation."
**STOP.** AskUserQuestion once per issue. Do NOT batch. Recommend + WHY. If no issues or fix is obvious, state what you'll do and move on — don't waste a question. Do NOT proceed until user responds.
...
- **Run `/mpm-plan-eng-review`** to lock in the engineering execution plan — architecture, tests, edge cases.
- The user can update `.mpm/docs/` foundation documents (PROJECT.md, ARCHITECTURE.md, etc.) based on the insights from this review.
