...
```
If a design doc exists (from `/mpm-office-hour`), read it. Use it as the source of truth for the problem statement, constraints, and chosen approach. If it has a `Supersedes:` field, note that this is a revised design — check the prior version for context on what changed and why.

### CEO Plan Check
```bash
CEO_PLAN=$(ls -t .mpm/gstack/ceo-plans/*.md 2>/dev/null | head -1)
[ -n "$CEO_PLAN" ] && echo "CEO plan found: $CEO_PLAN" || echo "No CEO plan found"
```
If a CEO plan exists (from `/mpm-plan-ceo-review`), read it. Note the mode (EXPANSION/SELECTIVE/HOLD/REDUCTION), accepted scope decisions, and any deferred items.

Also read `.mpm/docs/` documents if they exist — PROJECT.md, ARCHITECTURE.md, etc.
...
- The user can update `.mpm/docs/` foundation documents (PROJECT.md, ARCHITECTURE.md, etc.) based on the accumulated insights from the office-hour → CEO review → eng review chain.
- Implementation can begin — tasks should be created via `/mpm-task-write`.
