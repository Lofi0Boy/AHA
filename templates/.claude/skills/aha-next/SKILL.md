...
1. Run the pop command:
   ```bash
   python3 .aha/scripts/task.py pop ${CLAUDE_SESSION_ID}
   ```
2. If the queue is empty, inform the user.
3. Read the popped task from `.aha/data/current/${CLAUDE_SESSION_ID}.json`.
...
python3 .aha/scripts/task.py update ${CLAUDE_SESSION_ID} changes "file:lines (what changed), ..."
