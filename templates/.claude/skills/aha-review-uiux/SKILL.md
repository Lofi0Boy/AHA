---
name: aha-review-uiux
description: UI/UX unified review — design system compliance, aha-ui-ux-pro-max guideline check, and browser-based visual verification. UI tasks only.
---
...
> - **`.aha/docs/VERIFICATION.md`** — project-specific verification tools and exact commands. Use the **Browser Verification** section for navigation, screenshots, and interaction throughout this review.
> - **`.aha/docs/DESIGN.md`** + **`.aha/docs/tokens/`** — project design system. Primary criteria for visual judgment.
> - **`.aha/docs/UIUX.md`** — UI structure, screen flows, interaction states, user journey. Check if the implementation matches the screens, states, and flows defined here.
> - **`.aha/docs/PROJECT.md`** — product vision and target users. UX must serve them.

Three pillars:
1. Project design system compliance (DESIGN.md + tokens/)
2. Professional UX standards (/aha-ui-ux-pro-max guidelines)
...
For each finding: compare against token values in `.aha/docs/tokens/`. Hardcoded values that should be tokens → FAIL.
...
## 2. /aha-ui-ux-pro-max guideline alignment

Invoke `/aha-ui-ux-pro-max` to load professional UX standards. Then judge the implementation against them.
...
Judge against aha-ui-ux-pro-max Priority 1 rules. Key checks:
...
Judge against aha-ui-ux-pro-max Priority 2 rules. Key checks:
...
Judge against aha-ui-ux-pro-max Priority 5 rules. Take mobile + tablet screenshots:
...
Run the aha-ui-ux-pro-max pre-delivery checklist against the implementation. Any failure → FAIL.
...
Save all screenshots to `.aha/data/reviews/{task-id}-*.png`.
...
UX Standards (aha-ui-ux-pro-max):
...
Screenshots:
- .aha/data/reviews/{task-id}-desktop.png
- .aha/data/reviews/{task-id}-mobile.png
- .aha/data/reviews/{task-id}-interaction.png
```
