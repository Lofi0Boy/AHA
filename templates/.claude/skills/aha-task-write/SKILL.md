...
**NEVER write .aha/data/ JSON files directly.** Always use `task.py add`.
...
- **Ground in foundation docs.** Every task prompt must cite specific sections from `.aha/docs/` documents — not just mention them generically.

---

## On UI/UX Tasks

When a Task involves UI changes, `.aha/docs/DESIGN.md`, `.aha/docs/tokens/`, and `.aha/docs/UIUX.md` must be referenced.

**Include in Context:**
- Design tokens to apply (cite specific token names from `.aha/docs/tokens/`)
...
- "Follow `/aha-ui-ux-pro-max` skill UX guidelines for interaction patterns, accessibility, and animation."
- "Before marking done, run through the pre-delivery checklist in `/aha-ui-ux-pro-max` skill."

**If DESIGN.md or UIUX.md is missing or incomplete:**
- Do not create UI Tasks without design and UI/UX criteria
- Run `/aha-init-uiux` skill first

**If a Task needs tokens not yet defined in `.aha/docs/tokens/`:**
- Create new tokens that are **aligned with existing ones** (same scale, naming convention, color palette)
- Add the new tokens to the token file in `.aha/docs/tokens/`
...
```bash
python3 .aha/scripts/task.py add "task title" "## Context
- File paths to read
