...
description: |
  ...
  Use before /aha-plan-ceo-review or /aha-plan-eng-review.
---
...
- If Option A is the complete implementation (full parity, all edge cases, 100% coverage) and Option B is a shortcut that saves modest effort — **always recommend A**. The delta between 80 lines and 150 lines is meaningless with AI-assisted coding. "Good enough" is the wrong instinct when "complete" costs minutes more.
...
- BAD: "We can skip edge case handling to save time." (Edge case handling costs minutes with AI.)
...
1. Read `CLAUDE.md`, `.aha/docs/` documents (if they exist).
2. Run `git log --oneline -30` and `git diff origin/main --stat 2>/dev/null` to understand recent context.
3. Use Grep/Glob to map the codebase areas most relevant to the user's request.
4. **List existing product specs for this project:**
   ```bash
   ls -t .aha/gstack/design-*.md 2>/dev/null
   ```
...
```bash
grep -li "<keyword1>\|<keyword2>\|<keyword3>" .aha/gstack/design-*.md 2>/dev/null
```
...
> Tell the user: "I'll generate a quick wireframe sketch to help visualize the layout and functionality. This is intentionally rough — just boxes and labels, not design. Visual design (colors, typography, styling) will be handled separately later via `/aha-init-uiux`."
...
SKETCH_FILE=".aha/gstack/sketches/sketch-$(date +%s).html"
```
...
1. **gstack browse** (if available):
   ```bash
   gstack-browse goto "file://$(pwd)/$SKETCH_FILE" && gstack-browse screenshot .aha/gstack/sketches/sketch.png
   ```
2. **Headless Chrome** (if available):
   ```bash
   google-chrome --headless --screenshot=.aha/gstack/sketches/sketch.png --window-size=1280,900 "file://$(pwd)/$SKETCH_FILE"
   ```
...
Sketch files stay in `.aha/gstack/sketches/` for later reference (e.g., `/aha-init-uiux` will use them when defining UIUX.md).

In the product spec, describe the wireframe layout in text and reference the sketch path:
- "Wireframe sketch: `.aha/gstack/sketches/sketch-{timestamp}.html`"
...
```bash
PRIOR=$(ls -t .aha/gstack/design-*.md 2>/dev/null | head -1)
```
If `$PRIOR` exists, the new doc gets a `Supersedes:` field referencing it. This creates a revision chain — you can trace how a design evolved across office hours sessions.

Write to `.aha/gstack/design-{datetime}.md`:
...
Generated by /aha-office-hour on {date}
Branch: {branch}
Repo: {owner/repo}
Status: DRAFT
Mode: Startup
...
1. Suggest the next steps:
   - **`/aha-plan-ceo-review`** for ambitious features — rethink the problem, find the 10-star product, challenge scope
   - **`/aha-plan-eng-review`** for well-scoped implementation planning — lock in architecture, tests, edge cases

2. The product spec at `.aha/gstack/` is automatically discoverable by downstream skills — they will read it during their pre-review audit.

3. After completing the full review chain (`/aha-plan-ceo-review` → `/aha-plan-eng-review`), update `.aha/docs/` foundation documents (PROJECT.md, ARCHITECTURE.md, etc.) based on the accumulated design and review outputs.
